...
def benchmark_domain_encode_rep(
    ...
    *,
    ...
    """Benchmark representation candidates.

    quality_mode controls the candidate family:
      - "Exact": byte-exact recovery. Only wrappers/compressors that restore the
        original byte stream are allowed: keep, gzip, bz2, xz, and zip-single-file.
        No image/audio/video/document transcoding is allowed here.
      - "Lossless": content-level lossless transforms may be allowed. This is kept
        for compatibility, but exact_recovery should call "Exact".
      - "Lossy": content-preserving candidates are allowed where available.
    """
    dom = detect_domain(input_path, raw_bytes)
    m = detect_magic(raw_bytes)
    qmode = str(quality_mode or "Lossy").strip().lower()
    if qmode.startswith("exact"):
        qmode_norm = "Exact"
    elif qmode.startswith("lossless"):
        qmode_norm = "Lossless"
    else:
        qmode_norm = "Lossy"
    ...
    candidates.append(("keep", raw_bytes, {
        "domain": dom,
        "policy": keep_policy,
        "lossy": False,
        "exact_byte_recovery": True,
        "kind": _rep_kind(raw_bytes),
    }))

    def _add_byte_reversible(prefix: str, *, source_kind: Optional[str] = None):
        """Add only byte-exact reversible wrappers around the original byte stream."""
        for lvl in (1, 3, 6, 9):
            _try_add(
                f"gzip_lvl{lvl}",
                lambda lvl=lvl: _encode_gzip(raw_bytes, level=lvl),
                {"domain": dom, "policy": f"{prefix}_gzip{int(lvl)}", "lossy": False, "exact_byte_recovery": True, "source_kind": source_kind},
            )
        for lvl in (1, 3, 5, 7, 9):
            _try_add(
                f"bz2_lvl{lvl}",
                lambda lvl=lvl: _encode_bz2(raw_bytes, compresslevel=lvl),
                {"domain": dom, "policy": f"{prefix}_bz2_{int(lvl)}", "lossy": False, "exact_byte_recovery": True, "source_kind": source_kind},
            )
        for preset in (0, 3, 6, 9):
            _try_add(
                f"xz_p{preset}",
                lambda preset=preset: _encode_xz(raw_bytes, preset=preset),
                {"domain": dom, "policy": f"{prefix}_xz{int(preset)}", "lossy": False, "exact_byte_recovery": True, "source_kind": source_kind},
            )
        for lvl in (1, 3, 6, 9):
            _try_add(
                f"zip_lvl{lvl}",
                lambda lvl=lvl: zip_single_file(input_path, level=lvl)[0],
                {"domain": dom, "policy": f"{prefix}_zip{int(lvl)}", "lossy": False, "exact_byte_recovery": True, "source_kind": source_kind},
            )

    # Exact recovery: stop here after adding only byte-reversible compressors.
    # This intentionally excludes png_lossless, webp_lossless, flac_lossless,
    # OOXML rezip, and all lossy codecs because they do not reproduce the exact
    # original byte stream.
    if qmode_norm == "Exact":
        exact_prefix = (m.kind if (dom == "document" and m and m.kind) else dom)
        _add_byte_reversible(exact_prefix, source_kind=(m.kind if m else None))

    elif dom == "text":
        _add_byte_reversible("text")

    elif dom == "document":
        kind = m.kind if m else None
        is_pdf = bool(kind == "pdf")
        is_ooxml = bool(kind in {"docx", "pptx", "xlsx", "epub"})
        prefix = "pdf" if is_pdf else "document"
        _add_byte_reversible(prefix, source_kind=kind)
        # Content-level lossless optimization for OOXML is allowed outside Exact.
        # It can change the container bytes, so exact_recovery never reaches this branch.
        if is_ooxml:
            for lvl in (1, 3, 6, 9):
                _try_add(
                    f"rezip_lvl{lvl}",
                    lambda lvl=lvl: _rezip_zip_container(raw_bytes, level=lvl),
                    {"domain": dom, "policy": f"ooxml_rezip{int(lvl)}", "lossy": False, "exact_byte_recovery": False, "source_kind": kind},
                )

    elif dom == "archive":
        _add_byte_reversible("archive")

    elif dom == "binary":
        _add_byte_reversible("binary")

    elif dom == "image":
        if qmode_norm == "Lossless":
            _try_add("png_lossless", lambda: _encode_image_png(raw_bytes), {"domain": dom, "policy": "png_lossless", "lossy": False, "exact_byte_recovery": False})
            _try_add("webp_lossless", lambda: _encode_image_webp(raw_bytes, quality=100, lossless=True, allow_external_ffmpeg=allow_external_ffmpeg), {"domain": dom, "policy": "webp_lossless", "lossy": False, "exact_byte_recovery": False})
        if qmode_norm == "Lossy":
            _try_add("png_lossless", lambda: _encode_image_png(raw_bytes), {"domain": dom, "policy": "png_lossless", "lossy": False, "exact_byte_recovery": False})
            _try_add("webp_lossless", lambda: _encode_image_webp(raw_bytes, quality=100, lossless=True, allow_external_ffmpeg=allow_external_ffmpeg), {"domain": dom, "policy": "webp_lossless", "lossy": False, "exact_byte_recovery": False})
            for q in image_webp_qualities:
                _try_add(f"webp_q{int(q)}", lambda q=q: _encode_image_webp(raw_bytes, quality=int(q), lossless=False, allow_external_ffmpeg=allow_external_ffmpeg), {"domain": dom, "policy": f"webp_q{int(q)}", "lossy": True, "exact_byte_recovery": False, "webp_quality": int(q)})
            for q in image_jpeg_qualities:
                _try_add(f"jpeg_q{int(q)}", lambda q=q: _encode_image_jpeg(raw_bytes, quality=int(q)), {"domain": dom, "policy": f"jpeg_q{int(q)}", "lossy": True, "exact_byte_recovery": False, "jpeg_quality": int(q)})

    elif dom == "audio":
        if qmode_norm in {"Lossless", "Lossy"}:
            _try_add("flac_lossless", lambda: _encode_audio_flac(input_path, allow_external_ffmpeg=allow_external_ffmpeg), {"domain": dom, "policy": "flac_lossless", "lossy": False, "exact_byte_recovery": False})
        if qmode_norm == "Lossy":
            for br in opus_bitrates_kbps:
                _try_add(f"opus_{int(br)}k", lambda br=br: _encode_audio_opus(input_path, allow_external_ffmpeg=allow_external_ffmpeg, bitrate_kbps=int(br)), {"domain": dom, "policy": f"opus_{int(br)}", "lossy": True, "exact_byte_recovery": False, "opus_bitrate_kbps": int(br)})
            for br in mp3_bitrates_kbps:
                _try_add(f"mp3_{int(br)}k", lambda br=br: _encode_audio_mp3(input_path, allow_external_ffmpeg=allow_external_ffmpeg, bitrate_kbps=int(br)), {"domain": dom, "policy": f"mp3_{int(br)}", "lossy": True, "exact_byte_recovery": False, "mp3_bitrate_kbps": int(br)})
            for br in aac_bitrates_kbps:
                _try_add(f"aac_{int(br)}k", lambda br=br: _encode_audio_aac_m4a(input_path, allow_external_ffmpeg=allow_external_ffmpeg, bitrate_kbps=int(br)), {"domain": dom, "policy": f"aac_{int(br)}", "lossy": True, "exact_byte_recovery": False, "aac_bitrate_kbps": int(br)})

    elif dom == "video":
        if qmode_norm == "Lossy":
            for crf in video_crfs:
                _try_add(f"h264_crf{int(crf)}", lambda crf=crf: _encode_video_h264_mp4(input_path, allow_external_ffmpeg=allow_external_ffmpeg, crf=int(crf)), {"domain": dom, "policy": f"h264_crf{int(crf)}", "lossy": True, "exact_byte_recovery": False, "crf": int(crf)})
            for crf in video_vp9_crfs:
                _try_add(f"vp9_crf{int(crf)}", lambda crf=crf: _encode_video_vp9_webm(input_path, allow_external_ffmpeg=allow_external_ffmpeg, crf=int(crf)), {"domain": dom, "policy": f"vp9_crf{int(crf)}", "lossy": True, "exact_byte_recovery": False, "crf": int(crf)})
            for crf in video_av1_crfs:
                _try_add(f"av1_crf{int(crf)}", lambda crf=crf: _encode_video_av1_mkv(input_path, allow_external_ffmpeg=allow_external_ffmpeg, crf=int(crf)), {"domain": dom, "policy": f"av1_crf{int(crf)}", "lossy": True, "exact_byte_recovery": False, "crf": int(crf)})

    else:
        _add_byte_reversible(dom)

    # Headerless/self-describing pipeline ranking:
    # DNA payload is the representation bytes themselves, not zlib(DCP1+representation).
    # Therefore rank by rep_size_bytes. zlib_size_bytes is retained only as a legacy UI field.
    scored: list[Tuple[int, int, str, bytes, Dict[str, Any]]] = []
    for (name, rep_bytes, meta) in candidates:
        try:
            kind = _rep_kind(rep_bytes)
            if not kind:
                continue
            # Legacy/reference only; no longer used as payload size.
            try:
                z, _zmeta = zlib_wrap(rep_bytes, policy=str(zlib_policy or "auto"))
                zsz = len(z)
            except Exception:
                zsz = len(rep_bytes)
            scored.append((len(rep_bytes), zsz, name, rep_bytes, {**(meta or {}), "kind": kind}))
        except Exception:
            continue
    if not scored:
        raise RuntimeError("No self-describing representation candidates were produced.")

    scored.sort(key=lambda t: (t[0], t[1], t[2]))
    best_rep_size, best_zlib_size, best_name, best_bytes, best_meta = scored[0]

    candidates_sorted = []
    for (rsz, zsz, name, rep_bytes, meta) in scored:
        row = {
            "name": name,
            "rep_size_bytes": int(rsz),
            "zlib_size_bytes": int(zsz),  # legacy/reference only
            "payload_size_bytes": int(rsz),
            **(meta or {}),
        }
    ...
    bench_meta = {
        ...
        "exact_recovery_candidate_policy": "byte_reversible_only" if qmode_norm == "Exact" else "content_or_byte_reversible",
    ...
    best_meta.setdefault("exact_byte_recovery", qmode_norm == "Exact")
